...
public class TestFlinkLookupFunction extends FlinkCatalogTestBase {
  ...
  public void testLookupOptions() throws Exception {
    sql(
        "CREATE TABLE %s ("
            ...
            + "data string"
            + ") with ("
            + "'lookup-join-cache-ttl'='100000', "
            + "'lookup-join-cache-size'='10' "
            + ")",
        DTABLE_NAME);

    icebergtable = validationCatalog.loadTable(TableIdentifier.of(icebergNamespace, DTABLE_NAME));

    String ttl = icebergtable.properties().get("lookup-join-cache-ttl");
    String size = icebergtable.properties().get("lookup-join-cache-size");

    Assert.assertEquals("100000", ttl);
    Assert.assertEquals("10", size);
  }

  @Test
  public void testLookupJoinWithNoCache() throws Exception {
  ...
  @Test
  public void testLookupJoinWithCache() throws Exception {
    sql("CREATE TABLE %s (id int, data string)", TABLE_NAME);
    sql(
        "CREATE TABLE %s ("
            + "id int, "
  ...
  @Test
  public void testLookupjoinPartitionedTableWithNoCache() {
    sql("CREATE TABLE %s (id int, data string)", TABLE_NAME);
    sql(
        "CREATE TABLE %s ("
            + "id int, "
  ...
  @Test
  public void testLookupjoinPartitionedTableWithCache() {
    sql("CREATE TABLE %s (id int, data string)", TABLE_NAME);
    sql(
        "CREATE TABLE %s ("
            + "id int, "
